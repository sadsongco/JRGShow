...
import {
  outputParameters,
  outputParamVals,
} from "./modules/parameters/outputParameters.js";
...
const showParams = (modName) => {
  ...
  const params =
    modName === "Output" ? outputParameters : visualiserModules[modName].params;
  let paramContainer;
  ...
  for (let param of params) {
    ...
    switch (param.type) {
      case "val":
        ...
        paramEntry.value = param.hasOwnProperty("value")
          ? paramVals[param.name]
          : 100;
        paramEntry.step = param.hasOwnProperty("step") ? param.step : 1;
        break;
      case "toggle":
        paramEntry.type = "checkbox";
        paramEntry.checked = paramEntry.value = param.hasOwnProperty("value")
          ? paramVals[param.name]
          : false;
        break;
      case "colour":
...
const updateSlots = () => {
  ...
  for (const visIdx in currentVisChain) {
    ...
    currSlot =
      modSlots[parseInt(visIdx) + (modSlots.length - currentVisChain.length)];
  ...
  openRequest.onsuccess = () => {
    ...
    openCursorRequest.onsuccess = (event) => {
      ...
      saveRequest.onerror = (err) => {
        console.log(
          `Error saving setlist item: ${err.target.error.message}`,
          err
        );
      };
...
window.onload = async () => {
  ...
  if (urlParams.get("edit")) {
    ...
    document
      ...
      .appendChild(
        document.createTextNode(" - Change the Track Name to save a copy")
      );
  }
